...
from .models import Registration, Feedback, product, categories, cart, payments , orders
...
def vieworders(request):
    user = request.session["username"]
    k= orders.objects.filter(user=user)
    p = product.objects.filter(pid__in=Subquery(k.values('product_id')))
    return render(request, 'orders.html',{"products": p, "total_price": 0, "total_items": 0, "wallet": 0})
